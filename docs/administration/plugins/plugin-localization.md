...
1. Install your plugin into a local CRM (`POST /plugins/api/plugins/install`),
   enable it, and switch the admin user's UI locale to a language you
   shipped a translation for.
2. Load a page that uses a translated string. Confirm the translation renders.
3. Switch to a language you did **not** translate. Confirm the `en_US`
   fallback renders (this proves your fallback file is correct).
