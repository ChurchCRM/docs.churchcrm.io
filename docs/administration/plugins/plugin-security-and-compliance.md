---
title: Plugin Security & Compliance
sidebar_position: 3
---
...
| **high** | Send PII or financial data to a third party, react to donation hooks and call outbound APIs, expose its own HTTP routes, or ship binary blobs. High-risk plugins are reviewed by two maintainers before approval. |

Two specific combinations always escalate to **high** regardless of
how the plugin is described:

- `fs.write` + `network.outbound` — classic self-updater / supply-chain risk.
...
5. **Permission/capability inventory.** Every capability tag the
   plugin exercises is declared in `permissions`. Anything undeclared
...
1. **Disable the plugin immediately.** Click Disable in
   **Admin → Plugins**, or call `POST /plugins/api/plugins/{id}/disable`.
